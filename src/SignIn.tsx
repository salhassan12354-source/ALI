...
export default function SignIn() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (signInError) {
      ...
      // بعد تسجيل الدخول بنجاح، ننتقل للرئيسية
      navigate('/');
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-6 font-sans text-right" dir="rtl">
      ...
      >
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-black text-slate-800 dark:text-white mt-4">تسجيل الدخول</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm">أهلاً بك مجدداً في الخوارزمي AI</p>
        </div>
        ...
        <form onSubmit={handleSignIn} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center mr-2">
              <label className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">كلمة المرور</label>
              <a href="#" className="text-[10px] font-bold text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors underline">نسيت كلمة المرور؟</a>
            </div>
            <div className="relative group">
        ...
        <div className="text-center pt-2">
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
            ليس لديك حساب؟{' '}
            <Link to="/signup" className="text-slate-900 dark:text-white font-black hover:underline">
              أنشئ حساباً جديداً
            </Link>
          </p>
        </div>
